...
namespace DataAccess.Repositories
{
    public class UsersRepository : RepositoryBase<Models.User>, IUsersRepository
    {
        ...
        {
            var users = FindAll(false).ToList();
            var userSearched = users.Find(u => u.Name == user.Name);
            return userSearched.ToDomainModel();
        }
        public bool UserAvailable(User user)
        {
            var users = FindAll(false).ToList();
            var userSearched = users.Exists(u => u.Name == user.Name);
            return !userSearched;
        }
